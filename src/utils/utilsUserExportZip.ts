...
async function asdasd(media: IMedia, mediaIndex: number, isAlbumIndex: number = -1) {
    const mediaFiles = {}
    ...
    switch (media.type) {
        case 'album':
            ...
            for await (const albumItemMedia of media.list) {
                ...
                const albumMediaFiles = await asdasd(albumItemMedia, mediaIndex, iAlbumItem)
        ...
        case 'iframe':
            if (media.cover) {
                ...
                mediaFiles[`${rawMediaFileName}-cover.${getFileExtension(exportedMediaCover.file.name)}`]
                  = [
        ...
        case 'video':
            ...
            mediaFiles[`${rawMediaFileName}.${getFileExtension(exportedMedia.file.name)}`]
              = [
...
async function prepareUserZipConfig(user: IUser) {
    ...
    if (user.profile.avatar.isSet) {
        ...
        userZip[username][userConfigJson.profile.avatar] = [
          await fileToArrayBuffer(avatarFile), { level: 0 }
        ]
    }
